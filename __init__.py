...
from .maths import MakeTenBitUnsigned, TenBitUnsigned
...
Global_CPPHelper = (
    ctypes.cdll.LoadLibrary(Global_dllpath) if os.path.isfile(Global_dllpath) else None
)
...
TextureTypeLookup = {
    ...
    "basic": ("PBR", "Base Color", "Normal"),
    "basic+": ("PBR", "Base Color", "Normal"),
    "emissive": ("Normal/AO/Roughness", "Emission", "Base Color/Metallic"),
    "alphaclip": ("Normal/AO/Roughness", "Alpha Mask", "Base Color/Metallic"),
}
...
Global_Materials = (
    (
        "basic+",
        "Basic+",
        "A basic material with a color, normal, and PBR map which renders in the UI, Sourced from the super credits prop",
    ),
    (
        "alphaclip",
        "Alpha Clip",
        "A material that supports an alpha mask which does not render in the UI. Sourced from a skeleton pile",
    ),
    (
        "original",
        "Original",
        "The original template used for all mods uploaded to Nexus prior to the addon's public release, which is bloated with additional unnecessary textures. Sourced from a terminid",
    ),
    (
        "basic",
        "Basic",
        "A basic material with a color, normal, and PBR map. Sourced from a trash bag prop",
    ),
    (
        "emissive",
        "Emissive",
        "A basic material with a color, normal, and emission map. Sourced from a vending machine",
    ),
)
...
def CheckBlenderVersion():
    ...
    PrettyPrint(
        f"Blender Version: {BlenderVersion} Correct Version: {OnCorrectBlenderVersion}"
    )
...
def CheckAddonUpToDate():
    ...
    try:
        ...
        if req.status_code == requests.codes.ok:
            ...
            latestVersion = (
                int(latestVersion.split(".")[0]),
                int(latestVersion.split(".")[1]),
                int(latestVersion.split(".")[2]),
            )
            ...
            Global_latestAddonVersion = (
                f"{latestVersion[0]}.{latestVersion[1]}.{latestVersion[2]}"
            )
            ...
            PrettyPrint(
                f"Request Failed, Cannot check latest Version. Status: {req.status_code}",
                "warn",
            )
...
def GetMeshData(og_object):
    ...
    for idx in range(len(object.material_slots)):
        materials[idx].IDFromName(object.material_slots[idx].name)
            ...
            for group in vertex.groups:
                ...
                if group.weight > 0.001:
                    ...
                    if HDGroupIndex + 1 > len(boneIndices):
                        ...
                        boneIndices.extend(
                            [[[0, 0, 0, 0] for n in range(len(vertices))]] * dif
                        )
...
def CreateModel(model, id, customization_info, bone_names, transform_info, bone_info):
    ...
    for mesh in model:
        ...
        # check physics
        if (
            not bpy.context.scene.Hd2ToolPanelSettings.ImportPhysics
            and mesh.IsPhysicsBody()
        ):
            continue
        # check static
        if (
        ...
        new_object.scale = (
            mesh.DEV_Transform.scale[0],
            mesh.DEV_Transform.scale[1],
            mesh.DEV_Transform.scale[2],
        )
        new_object.location = (
            mesh.DEV_Transform.pos[0],
            mesh.DEV_Transform.pos[1],
            mesh.DEV_Transform.pos[2],
        )
            ...
            if mesh.DEV_BoneInfo is not None:
                for Bone in mesh.DEV_BoneInfo.Bones:
                    ...
                    bpy.ops.object.empty_add(
                        ...
                        radius=0.08,
                        align="WORLD",
                        location=(current_pos[0], current_pos[1], current_pos[2]),
                        scale=(1, 1, 1),
                    )
...
def NormalsFromPalette(normals):
    ...
    c_output = (ctypes.c_char * len(output)).from_buffer(output)
    Global_CPPHelper.dll_NormalsFromPalette(
        c_output, c_normals, ctypes.c_uint32(len(normals))
    )
    F = MemoryStream(output, IOMode="read")
...
def LoadArchiveHashes():
    ...
    for title in data:
        for innerKey in data[title]:
            Global_ArchiveHashes.append(
                [innerKey, title + ": " + data[title][innerKey]]
            )
...
class TocEntry:
    def __init__(self):
        self.FileID = self.TypeID = self.TocDataOffset = self.Unknown1 = (
            self.GpuResourceOffset
        ) = self.Unknown2 = self.TocDataSize = self.GpuResourceSize = (
            self.EntryIndex
        ) = self.StreamSize = self.StreamOffset = 0
        self.Unknown3 = 16
        self.Unknown4 = 64
    ...
    def SerializeData(self, TocFile, GpuFile, StreamFile):
        ...
        if GpuFile.IsWriting():
            self.GpuResourceOffset = ceil(float(GpuFile.tell()) / 64) * 64
        ...
        if StreamFile.IsWriting():
            self.StreamOffset = ceil(float(StreamFile.tell()) / 64) * 64
        if self.StreamSize > 0:
            StreamFile.seek(self.StreamOffset)
            if StreamFile.IsReading():
    ...
    def Load(self, Reload=False, MakeBlendObject=True):
        ...
        if self.TypeID == MeshID:
            callback = LoadStingrayMesh
        if self.TypeID == TexID:
            callback = LoadStingrayTexture
        if self.TypeID == MaterialID:
            callback = LoadStingrayMaterial
        if self.TypeID == CompositeMeshID:
...
class StreamToc:
    ...
    def Serialize(self, SerializeData=True):
        ...
        if self.TocFile.IsReading():
            self.TocEntries = [
                Entry.Serialize(self.TocFile) for Entry in self.TocEntries
            ]
        else:
            Index = 1
...
class TocManager:
    ...
    def LoadArchive(self, path, SetActive=True, IsPatch=False):
        ...
        if SetActive and not IsPatch:
            unloadEmpty = (
                bpy.context.scene.Hd2ToolPanelSettings.UnloadEmptyArchives
                and bpy.context.scene.Hd2ToolPanelSettings.EnableTools
            )
            if unloadEmpty:
            ...
            for entry in self.ActivePatch.TocEntries:
                if entry.TypeID == MaterialID:
                    ...
                    if ID in Global_MaterialParentIDs:
                        ...
                        entry.Load()
                        PrettyPrint(
                            f"Creating Material: {entry.FileID} Template: {entry.MaterialTemplate}"
                        )
                    else:
                        PrettyPrint(
                            f"Material: {entry.FileID} Parent ID: {ID} is not an custom material, skipping."
                        )
...
class ShaderVariable:
    klasses = {0: "Scalar", 1: "Vector2", 2: "Vector3", 3: "Vector4", 12: "Other"}

    def __init__(self):
        self.klass = self.klassName = self.elements = self.ID = self.offset = (
            self.elementStride
        ) = 0
        self.values = []
        self.name = ""
...
class StingrayMaterial:
    def __init__(self):
        self.undat1 = self.undat3 = self.undat4 = self.undat5 = self.undat6 = (
            self.RemainingData
        ) = bytearray()
        self.EndOffset = self.undat2 = self.ParentMaterialID = self.NumTextures = (
            self.NumVariables
    ...
    def Serialize(self, f):
        ...
        for variable in self.ShaderVariables:
            ...
            if f.IsReading():
                variable.values = [
                    0 for n in range(variable.klass + 1)
                ]  # Create an array with the length of the data which is one greater than the klass value

        variableValueLocation = f.Location  # Record and add all of the extra data that is skipped around during the variable offsets
        if f.IsReading():
            self.RemainingData = f.bytes(self.RemainingData, len(f.Data) - f.tell())
        if f.IsWriting():
            self.RemainingData = f.bytes(self.RemainingData)
...
def LoadStingrayMaterial(ID, TocData, GpuData, StreamData, Reload, MakeBlendObject):
    ...
    if MakeBlendObject and not (exists and not Reload):
        AddMaterialToBlend(ID, Material, Reload)
    elif force_reload:
        AddMaterialToBlend(ID, Material, True)
    return Material
...
def CreateAddonMaterial(ID, StingrayMat, mat, Entry):
    ...
    if Entry.MaterialTemplate == "basic":
        SetupBasicBlenderMaterial(
            nodeTree, inputNode, outputNode, bsdf, separateColor, normalMap
        )
    elif Entry.MaterialTemplate == "basic+":
        SetupBasicBlenderMaterial(
            nodeTree, inputNode, outputNode, bsdf, separateColor, normalMap
        )
    elif Entry.MaterialTemplate == "original":
        SetupOriginalBlenderMaterial(
            nodeTree, inputNode, outputNode, bsdf, separateColor, normalMap
        )
    elif Entry.MaterialTemplate == "emissive":
        SetupEmissiveBlenderMaterial(
            nodeTree, inputNode, outputNode, bsdf, separateColor, normalMap
        )
    elif Entry.MaterialTemplate == "alphaclip":
        SetupAlphaClipBlenderMaterial(
            nodeTree, inputNode, outputNode, bsdf, separateColor, normalMap, mat
        )


def SetupBasicBlenderMaterial(
    nodeTree, inputNode, outputNode, bsdf, separateColor, normalMap
):
...
def SetupOriginalBlenderMaterial(
    nodeTree, inputNode, outputNode, bsdf, separateColor, normalMap
):
...
def SetupEmissiveBlenderMaterial(
    nodeTree, inputNode, outputNode, bsdf, separateColor, normalMap
):
    nodeTree.links.new(
        inputNode.outputs["Base Color/Metallic"], bsdf.inputs["Base Color"]
    )
    nodeTree.links.new(inputNode.outputs["Emission"], bsdf.inputs["Emission Color"])
    nodeTree.links.new(
        inputNode.outputs["Normal/AO/Roughness"], separateColor.inputs["Color"]
    )
...
def SetupAlphaClipBlenderMaterial(
    nodeTree, inputNode, outputNode, bsdf, separateColor, normalMap, mat
):
    bsdf.inputs["Emission Strength"].default_value = 0
...
def SetupNormalMapTemplate(nodeTree, inputNode, normalMap, bsdf):
    ...
    nodeTree.links.new(
        separateColorNormal.outputs["Red"], combineColorNormal.inputs["Red"]
    )
    nodeTree.links.new(
        separateColorNormal.outputs["Green"], combineColorNormal.inputs["Green"]
    )
...
def LoadStingrayTexture(ID, TocData, GpuData, StreamData, Reload, MakeBlendObject):
    ...
    StingrayTex = StingrayTexture()
    StingrayTex.Serialize(
        MemoryStream(TocData), MemoryStream(GpuData), MemoryStream(StreamData)
    )
    ...
    if MakeBlendObject and not (exists and not Reload):
        ...
        if os.path.isfile(png_path):
            ...
            raise Exception(
                f"Failed to convert texture {ID} to PNG, or DDS failed to export"
            )
    ...
    def Serialize(self, f):
        ...
        if f.IsReading():
            ...
            if len(self.BoneHashes) == len(self.Names):
                ...
                PrettyPrint(
                    f"Failed to add bone hashes as list length is misaligned. Hashes Length: {len(self.BoneHashes)} Names Length: {len(self.Names)} Hashes: {self.BoneHashes} Names: {self.Names}",
                    "error",
                )
        return self
...
class StingrayCompositeMesh:
    ...
    def Serialize(self, f, gpu):
        ...
        if f.IsWriting():
            ...
            self.StreamInfoUnk = [
                mesh_info.MeshID for mesh_info in self.MeshInfoArray[: self.NumStreams]
            ]
        if f.IsReading():
            self.StreamInfoOffsets = [0 for n in range(self.NumStreams)]
        ...
        for stream_idx in range(self.NumStreams):
            if f.IsReading():
                ...
                self.StreamInfoOffsets[stream_idx] = f.tell() - self.StreamInfoOffset
...
def LoadStingrayCompositeMesh(
    ID, TocData, GpuData, StreamData, Reload, MakeBlendObject
):
...
class TransformInfo:  # READ ONLY
    ...
    def Serialize(self, f):
        ...
        self.PositionTransforms = [
            StingrayLocalTransform().SerializeV2(f) for n in range(self.NumTransforms)
        ]
        self.TransfromEntries = [
...
class MeshSectionInfo:
    def __init__(self, ID=0):
        self.unk1 = self.VertexOffset = self.NumVertices = self.IndexOffset = (
            self.NumIndices
        ) = self.unk2 = 0
...
class MeshInfo:
    ...
    def Serialize(self, f):
        ...
        if f.IsReading():
            self.SectionIDs = [0 for n in range(self.NumSections)]
        else:
            self.SectionIDs = [section.ID for section in self.Sections]
        self.SectionIDs = [f.uint32(ID) for ID in self.SectionIDs]
        if f.IsReading():
            self.Sections = [
                MeshSectionInfo(self.SectionIDs[n]) for n in range(self.NumSections)
            ]
...
class StingrayMeshFile:
    def __init__(self):
        ...
        self.CustomizationInfoOffset = self.UnkHeaderOffset1 = self.UnkHeaderOffset2 = (
            self.TransformInfoOffset
        ) = self.UnkRef1 = self.BonesRef = self.CompositeRef = 0
        ...
        for mesh in OrderedMeshes[stream_idx][0]:
            ...
            for vidx in range(len(mesh.VertexPositions)):
                ...
                for Component in Stream_Info.Components:
                    ...
                    # Get Vertex Position
                    if type == "position":
                        pos = Component.SerializeComponent(
                    ...
                    # Get Vertex Normal
                        ...
                        norm = Component.SerializeComponent(
                            gpu, mesh.VertexNormals[vidx]
                        )
                    ...
                    # Get Vertex UVs
                        ...
                        uv = Component.SerializeComponent(
                    ...
                    # Get Weights
                        ...
                        if (
                            ...
                            PrettyPrint(
                                "Multiple weight indices are unsupported!", "warn"
                            )
            ...
            for mesh in self.RawMeshes:
                ...
                if mesh not in VertOrderedMeshes_flat:
                    if smallest_vert_mesh is None:
                        ...
                        smallest_mesh_info = self.MeshInfoArray[
                            self.DEV_MeshInfoMap[smallest_vert_mesh.MeshInfoIndex]
                        ]
                        if (
                            mesh_info.Sections[0].VertexOffset
                            < smallest_mesh_info.Sections[0].VertexOffset
                        ):
                            smallest_vert_mesh = mesh
                ...
                if mesh not in IndexOrderedMeshes_flat:
                    if smallest_index_mesh is None:
                        ...
                        smallest_mesh_info = self.MeshInfoArray[
                            self.DEV_MeshInfoMap[smallest_index_mesh.MeshInfoIndex]
                        ]
                        if (
                            mesh_info.Sections[0].IndexOffset
                            < smallest_mesh_info.Sections[0].IndexOffset
                        ):
                            smallest_index_mesh = mesh
            mesh_info = self.MeshInfoArray[
                self.DEV_MeshInfoMap[smallest_vert_mesh.MeshInfoIndex]
            ]
    ...
    def InitRawMeshes(self):
        for n in range(len(self.MeshInfoArray)):
            ...
            NewMesh.DEV_Transform = self.TransformInfo.Transforms[
    ...
    def SetupRawMeshComponents(self, OrderedMeshes):
        for stream_idx in range(len(OrderedMeshes)):
            ...
            if (
                IsSkinned
                and NumBoneIndices > 1
                and bpy.context.scene.Hd2ToolPanelSettings.Force1Group
            ):
                NumBoneIndices = 1
...
def LoadStingrayMesh(ID, TocData, GpuData, StreamData, Reload, MakeBlendObject):
    ...
    if MakeBlendObject:
        CreateModel(
            StingrayMesh.RawMeshes,
            str(ID),
            StingrayMesh.CustomizationInfo,
            StingrayMesh.BoneNames,
            StingrayMesh.TransformInfo,
            StingrayMesh.BoneInfoArray,
        )
    return StingrayMesh
        ...
        if len(obj.keys()) > 1:
            for key in obj.keys():
                if key == "Z_ObjectID":
                    ...
                    PrettyPrint(
                        f"obj: {obj.name} id: {obj[key]} index: {obj[indexKey]}"
                    )
                    for otherObject in CustomObjects:
                        if (
                            obj[key] == otherObject[0]
                            and obj[indexKey] == otherObject[1]
                        ):
...
def IncorrectVertexGroupNaming(self):
    for obj in bpy.context.selected_objects:
        ...
        if len(groups) != len(obj.vertex_groups):
            self.report(
                {"WARNING"},
                f"Object: {obj.name} has a different number of vertex groups than expected",
            )
            return False
    return False
...
def ObjectHasModifiers(self):
    for obj in bpy.context.selected_objects:
        if obj.modifiers:
            self.report(
                {"ERROR"},
                f"Object: {obj.name} has {len(obj.modifiers)} unapplied modifiers",
            )
            return True
    return False
    ...
    for obj in bpy.context.selected_objects:
        ...
        if (
            ...
            if obj.location != transforms[0]:
                PrettyPrint(
                    f"Object Location: {obj.location} Expected Location: {transforms[0]}"
                )
            if obj.rotation_euler != transforms[1]:
                PrettyPrint(
                    f"Object Rotation: {obj.rotation_euler} Expected Rotation: {transforms[1]}"
                )
            if obj.scale != transforms[2]:
                PrettyPrint(
                    f"Object Scale: {obj.scale} Expected Scale: {transforms[2]}"
                )
            self.report(
                {"ERROR"}, f"Object: {obj.name} has mismatched transforms. See Console"
            )
            return True
    return False
...
def MaterialsNumberNames(self):
    ...
    for mesh in mesh_objs:
        ...
        if invalidMaterials > 0:
            self.report(
                {"ERROR"},
                f"Object: {mesh.name} has {invalidMaterials} non Helldivers 2 Materials",
            )
            return True
    return False
...
def MeshNotValidToSave(self):
    return (
        PatchesNotLoaded(self)
        or DuplicateIDsInScene(self)
        or IncorrectVertexGroupNaming(self)
        or ObjectHasModifiers(self)
        or MaterialsNumberNames(self)
        or HasZeroVerticies(self)
        or ObjectHasShapeKeys(self)
    )
                ...
                for entry in toc.TocEntries:
                    for fileID in fileIDs:
                        ...
                        if str(entry.FileID) == ID:
                            ...
                            PrettyPrint("")
                            PrettyPrint(
                                f"Archive: {filename} Entry ID: {ID} Name: {name}"
                            )
                            ...
                            PrettyPrint(
                                f"Overall Time: {hours} hrs {minutes} min {seconds} sec"
                            )
                            ...
                            if not forceSearch:
                                ...
                                PrettyPrint(
                                    f"Found {foundIDs} total occurrences of any IDs\n"
                                )
                            if len(fileIDs) == 0:
                                PrettyPrint(
                                    f"Finished Searching {searched}/{len(directorys)} files"
                                )
                                curenttime = (
                                ...
                                outputfile = (
                                    f"{Global_searchpath}output_{curenttime}.txt"
                                )
...
class ChangeFilepathOperator(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if steamapps in filepath:
            ...
            self.report(
                {"ERROR"}, f"Could not find steamapps folder in filepath: {filepath}"
            )
            return {"CANCELLED"}
...
class DefaultLoadArchiveOperator(Operator):
    ...
    def execute(self, context):
        ...
        if not os.path.exists(path):
            self.report(
                {"ERROR"}, "Current Filepath is Invalid. Change this in the Settings"
            )
    ...
    def execute(self, context):
        ...
        for filepath in filepaths:
            if (
                ...
                or filepath.endswith(".ini")
                or filepath.endswith(".data")
            ):
                continue
            path = Path(filepath)
            if not path.suffix.startswith(".patch_"):
                path = path.with_suffix("")

            archiveToc = Global_TocManager.LoadArchive(str(path), True, self.is_patch)
        PrettyPrint(
            f"Loaded {len(Global_TocManager.LoadedArchives) - oldLoadedLength} Archive(s)"
        )
    ...
    def execute(self, context):
        for index in range(len(Global_TocManager.LoadedArchives)):
            if (
                ...
                bpy.context.scene.Hd2ToolPanelSettings.LoadedArchives = (
                    Global_TocManager.LoadedArchives[nextIndex].Name
                )
                return {"FINISHED"}
        return {"CANCELLED"}
        ...
        if event.shift:
            if Global_TocManager.LastSelected is not None:
                ...
                if StartIndex > EndIndex:
                    Global_TocManager.SelectEntries(
                        Global_TocManager.DrawChain[EndIndex : StartIndex + 1], True
                    )
                else:
                    Global_TocManager.SelectEntries(
                        Global_TocManager.DrawChain[StartIndex : EndIndex + 1], True
                    )
            else:
                Global_TocManager.SelectEntries([Entry], True)
            return {"FINISHED"}
...
class MaterialShaderVariableEntryOperator(Operator):
    ...
    def execute(self, context):
        ...
        if Entry:
            Entry.LoadedData.ShaderVariables[self.variable_index].values[
                self.value_index
            ] = self.value
            PrettyPrint(
                f"Set value to: {self.value} at variable: {self.variable_index} value: {self.value_index} for material ID: {self.object_id}"
            )
        else:
...
class MaterialShaderVariableColorEntryOperator(Operator):
    ...
    def execute(self, context):
        ...
        if Entry:
            for idx in range(3):
                Entry.LoadedData.ShaderVariables[self.variable_index].values[idx] = (
                    self.color[idx]
                )
    ...
    def invoke(self, context, event):
        ...
        if Entry:
            for idx in range(3):
                self.color[idx] = Entry.LoadedData.ShaderVariables[
                    self.variable_index
                ].values[idx]
        else:
            self.report({"ERROR"}, f"Could not find entry for ID: {self.object_id}")
...
class DuplicateEntryOperator(Operator):
    ...
    def execute(self, context):
        ...
        if self.NewFileID == "":
            ...
            return {"CANCELLED"}
        Global_TocManager.DuplicateEntry(
            int(self.object_id), int(self.object_typeid), int(self.NewFileID)
        )
        Global_randomID = ""
        return {"FINISHED"}
...
def SaveMeshMaterials(objects):
    ...
    for material in materials:
        ...
        if (
            nodeName == ""
            and not bpy.context.scene.Hd2ToolPanelSettings.SaveNonSDKMaterials
        ):
        ...
        if entry:
            if not entry.IsModified:
                ...
                PrettyPrint(
                    f"Skipping Saving Material: {ID} as it already has been modified"
                )
        elif "-" in nodeName:
...
class BatchExportTexturePNGOperator(Operator):
    ...
    def execute(self, context):
        ...
        self.report(
            {"INFO"}, f"Exported {exportedfiles} PNG Files To: {self.directory}"
        )
        return {"FINISHED"}
...
class SearchArchivesOperator(Operator):
    ...
    def draw(self, context):
        ...
        if self.SearchField != "" and len(self.ArchivesToDisplay) == 0:
            row = layout.row()
            row.label(text="No Archive IDs Found")
            row = layout.row()
            row.label(text="Know an ID that's Not Here?")
            row = layout.row()
            row.label(text="Make an issue on the github.")
            row = layout.row()
            row.label(text="Archive ID and In Game Name")
            row = layout.row()
            row.operator("helldiver2.github", icon="URL")
            ...
            for Archive in self.ArchivesToDisplay:
                ...
                row.operator(
                    "helldiver2.archives_import", icon="FILE_NEW", text=""
                ).paths_str = Global_gamepath + str(Archive[0])
...
class LatestReleaseOperator(Operator):
    ...
    def execute(self, context):
        url = (
            "https://github.com/Boxofbiscuits97/HD2SDK-CommunityEdition/releases/latest"
        )
        webbrowser.open(url, new=0, autoraise=True)
        return {"FINISHED"}
...
class CopyCustomPropertyOperator(Operator):
    ...
    def execute(self, context):
        ...
        self.report(
            {"INFO"}, f"Copied {len(stored_custom_properties)} custom properties"
        )
        return {"FINISHED"}
...
class PasteCustomPropertyOperator(Operator):
    ...
    def execute(self, context):
        ...
        self.report(
            {"INFO"}, f"Pasted {len(stored_custom_properties)} custom properties"
        )
        return {"FINISHED"}
...
def LoadedArchives_callback(scene, context):
    return [
        (
            Archive.Name,
            GetArchiveNameFromID(Archive.Name)
            if GetArchiveNameFromID(Archive.Name) != ""
            else Archive.Name,
            Archive.Name,
        )
        for Archive in Global_TocManager.LoadedArchives
...
def Patches_callback(scene, context):
    return [
        (Archive.Name, Archive.Name, Archive.Name)
        for Archive in Global_TocManager.Patches
    ]
...
class HellDivers2ToolsPanel(Panel):
        ...
        if Entry.IsLoaded:
            ...
            if mat.DEV_ShowEditor:
                for i, t in enumerate(mat.TexIDs):
                    ...
                    row.operator(
                        "helldiver2.material_texture_entry",
                        icon="FILE_IMAGE",
                        text=label,
                        emboss=False,
                    ).object_id = str(t)
                ...
                for i, variable in enumerate(mat.ShaderVariables):
                    ...
                    if sections == 3:
                        sections = 4  # add an extra for the color picker
                    row = row.split(factor=1 / sections)
                    for j, value in enumerate(variable.values):
                        ShaderVariable = row.operator(
                            "helldiver2.material_shader_variable",
                    ...
                    if len(variable.values) == 3:
                        ColorPicker = row.operator(
                            "helldiver2.material_shader_variable_color",
                            text="",
                            icon="EYEDROPPER",
                        )
    ...
    def draw_entry_buttons(self, box, row, Entry, PatchOnly):
        if Entry.TypeID == MeshID:
            row.operator(
                "helldiver2.archive_mesh_save", icon="FILE_BLEND", text=""
            ).object_id = str(Entry.FileID)
            row.operator(
                "helldiver2.archive_mesh_import", icon="IMPORT", text=""
            ).object_id = str(Entry.FileID)
        elif Entry.TypeID == TexID:
            row.operator(
                "helldiver2.texture_saveblendimage", icon="FILE_BLEND", text=""
            ).object_id = str(Entry.FileID)
            row.operator(
                "helldiver2.texture_import", icon="IMPORT", text=""
            ).object_id = str(Entry.FileID)
        elif Entry.TypeID == MaterialID:
            row.operator(
                "helldiver2.material_save", icon="FILE_BLEND", text=""
            ).object_id = str(Entry.FileID)
            row.operator(
                "helldiver2.material_import", icon="IMPORT", text=""
            ).object_id = str(Entry.FileID)
            row.operator(
                "helldiver2.material_showeditor", icon="MOD_LINEART", text=""
            ).object_id = str(Entry.FileID)
            self.draw_material_editor(Entry, box, row)
        if Global_TocManager.IsInPatch(Entry):
            props = row.operator(
                "helldiver2.archive_removefrompatch", icon="FAKE_USER_ON", text=""
            )
            props.object_id = str(Entry.FileID)
            props.object_typeid = str(Entry.TypeID)
        else:
            props = row.operator(
                "helldiver2.archive_addtopatch", icon="FAKE_USER_OFF", text=""
            )
            props.object_id = str(Entry.FileID)
            props.object_typeid = str(Entry.TypeID)
        if Entry.IsModified:
        ...
        if PatchOnly:
            props = row.operator(
                "helldiver2.archive_removefrompatch", icon="X", text=""
            )
            props.object_id = str(Entry.FileID)
            props.object_typeid = str(Entry.TypeID)
    ...
    def draw(self, context):
        ...
        if bpy.app.version[1] > 0:
            row.label(
                text="Warning! Soft Supported Blender Version. Issues may Occur.",
                icon="ERROR",
            )
        ...
        if scene.Hd2ToolPanelSettings.MenuExpanded:
            ...
            if scene.Hd2ToolPanelSettings.EnableTools:
                ...
                # row.label()
                row.label(
                    text="WARNING! Developer Tools, Please Know What You Are Doing!"
                )
        ...
        if scene.Hd2ToolPanelSettings.EnableTools:
            ...
            row.scale_x = 1
        row.operator(
            "helldiver2.archive_import", icon="FILEBROWSER", text=""
        ).is_patch = False
        row = layout.row()
        if len(Global_TocManager.LoadedArchives) > 0:
        ...
        row.operator(
            "helldiver2.archive_createpatch", icon="COLLECTION_NEW", text="New Patch"
        )
        ...
        if scene.Hd2ToolPanelSettings.ContentsExpanded:
            ...
            row = layout.row()
            row.prop(
                scene.Hd2ToolPanelSettings, "SearchField", icon="VIEWZOOM", text=""
            )
            ...
            for Type in DisplayTocTypes:
                ...
                if show is None:
                    ...
                    if (
                        ...
                        or Type.TypeID == TexID
                        or Type.TypeID == MeshID
                    ):
                        fold = True
                ...
                sub.operator(
                    "helldiver2.collapse_section",
                    text=f"{typeName}: {str(Type.TypeID)}",
                    icon=fold_icon,
                    emboss=False,
                ).type = str(Type.TypeID)
                ...
                sub.operator(
                    "helldiver2.select_type", icon="RESTRICT_SELECT_OFF", text=""
                ).object_typeid = str(Type.TypeID)
                # Draw Add Material Button

                if typeName == "material":
                    sub.operator("helldiver2.material_add", icon="FILE_NEW", text="")
                ...
                for EntryInfo in DisplayTocEntries:
                    ...
                    if (
                        len(Global_TocManager.SavedFriendlyNameIDs) > len(DrawChain)
                        and Global_TocManager.SavedFriendlyNameIDs[len(DrawChain)]
                        == Entry.FileID
                    ):
                        FriendlyName = Global_TocManager.SavedFriendlyNames[
                            len(DrawChain)
                        ]
                    else:
                        try:
                            FriendlyName = Global_TocManager.SavedFriendlyNames[
                                Global_TocManager.SavedFriendlyNameIDs.index(
                                    Entry.FileID
                                )
                            ]
                    ...
                    # Draw Entry
                    PatchEntry = Global_TocManager.GetEntry(
                        int(Entry.FileID), int(Entry.TypeID)
                    )
                    ...
                    row.separator()
                    props = row.operator(
                        "helldiver2.archive_entry",
                        icon=type_icon,
                        text=FriendlyName,
                        emboss=PatchEntry.IsSelected,
                        depress=PatchEntry.IsSelected,
                    )
...
class WM_MT_button_context(Menu):
    ...
    def draw_entry_buttons(self, row, Entry):
        ...
        RemoveFromPatchName = (
            "Remove From Patch" if SingleEntry else f"Remove {NumSelected} From Patch"
        )
        AddToPatchName = (
            "Add To Patch" if SingleEntry else f"Add {NumSelected} To Patch"
        )
        ImportMeshName = (
            "Import Mesh" if SingleEntry else f"Import {NumSelected} Meshes"
        )
        ImportTextureName = (
            "Import Texture" if SingleEntry else f"Import {NumSelected} Textures"
        )
        ImportMaterialName = (
            "Import Material" if SingleEntry else f"Import {NumSelected} Materials"
        )
        DumpObjectName = (
            "Export Object Dump"
            if SingleEntry
            else f"Export {NumSelected} Object Dumps"
        )
        ImportDumpObjectName = (
            "Import Object Dump"
            if SingleEntry
            else f"Import {NumSelected} Object Dumps"
        )
        SaveTextureName = (
            "Save Blender Texture"
            if SingleEntry
            else f"Save Blender {NumSelected} Textures"
        )
        SaveMaterialName = (
            "Save Material" if SingleEntry else f"Save {NumSelected} Materials"
        )
        UndoName = (
            "Undo Modifications" if SingleEntry else f"Undo {NumSelected} Modifications"
        )
        ...
        if len(Global_TocManager.CopyBuffer) != 0:
            row.operator(
                "helldiver2.archive_paste",
                icon="PASTEDOWN",
                text="Paste " + str(len(Global_TocManager.CopyBuffer)) + " Entries",
            )
            row.operator(
                "helldiver2.archive_clearclipboard",
                icon="TRASH",
                text="Clear Clipboard",
            )
        if SingleEntry:
            props = row.operator(
                "helldiver2.archive_duplicate", icon="DUPLICATE", text="Duplicate Entry"
            )
            props.object_id = str(Entry.FileID)
            props.object_typeid = str(Entry.TypeID)
        ...
        if Global_TocManager.IsInPatch(Entry):
            props = row.operator(
                "helldiver2.archive_removefrompatch", icon="X", text=RemoveFromPatchName
            )
            props.object_id = FileIDStr
            props.object_typeid = TypeIDStr
        else:
            props = row.operator(
                "helldiver2.archive_addtopatch", icon="PLUS", text=AddToPatchName
            )
            props.object_id = FileIDStr
            props.object_typeid = TypeIDStr
        ...
        if AreAllMeshes:
            row.operator(
                "helldiver2.archive_mesh_import", icon="IMPORT", text=ImportMeshName
            ).object_id = FileIDStr
        if AreAllTextures:
            row.operator(
                "helldiver2.texture_import", icon="IMPORT", text=ImportTextureName
            ).object_id = FileIDStr
        elif AreAllMaterials:
            row.operator(
                "helldiver2.material_import", icon="IMPORT", text=ImportMaterialName
            ).object_id = FileIDStr
        # Draw export buttons
        row.separator()
        if SingleEntry:
            props = row.operator(
                "helldiver2.archive_object_dump_import",
                icon="PACKAGE",
                text=ImportDumpObjectName,
            )
            props.object_id = FileIDStr
            props.object_typeid = TypeIDStr
        props = row.operator(
            "helldiver2.archive_object_dump_export", icon="PACKAGE", text=DumpObjectName
        )
        props.object_id = FileIDStr
        props.object_typeid = TypeIDStr
        ...
        if AreAllMeshes:
            if SingleEntry:
                row.operator(
                    "helldiver2.archive_mesh_save", icon="FILE_BLEND", text="Save Mesh"
                ).object_id = str(Entry.FileID)
            else:
                row.operator(
                    "helldiver2.archive_mesh_batchsave",
                    icon="FILE_BLEND",
                    text=f"Save {NumSelected} Meshes",
                )
        elif AreAllTextures:
            row.operator(
                "helldiver2.texture_saveblendimage",
                icon="FILE_BLEND",
                text=SaveTextureName,
            ).object_id = FileIDStr
            if SingleEntry:
                row.separator()
                row.operator(
                    "helldiver2.texture_savefromdds",
                    icon="FILE_IMAGE",
                    text="Import DDS Texture",
                ).object_id = str(Entry.FileID)
                row.operator(
                    "helldiver2.texture_savefrompng",
                    icon="FILE_IMAGE",
                    text="Import PNG Texture",
                ).object_id = str(Entry.FileID)
                row.separator()
                row.operator(
                    "helldiver2.texture_export",
                    icon="OUTLINER_OB_IMAGE",
                    text="Export DDS Texture",
                ).object_id = str(Entry.FileID)
                row.operator(
                    "helldiver2.texture_export_png",
                    icon="OUTLINER_OB_IMAGE",
                    text="Export PNG Texture",
                ).object_id = str(Entry.FileID)
            else:
                row.separator()
                row.operator(
                    "helldiver2.texture_batchexport",
                    icon="OUTLINER_OB_IMAGE",
                    text=f"Export {NumSelected} DDS Textures",
                ).object_id = FileIDStr
                row.operator(
                    "helldiver2.texture_batchexport_png",
                    icon="OUTLINER_OB_IMAGE",
                    text=f"Export {NumSelected} PNG Textures",
                ).object_id = FileIDStr
        elif AreAllMaterials:
            row.operator(
                "helldiver2.material_save", icon="FILE_BLEND", text=SaveMaterialName
            ).object_id = FileIDStr
            if SingleEntry:
                row.operator(
                ...
                if Entry.LoadedData is not None:
                    row.operator(
                        "helldiver2.copytest",
                        icon="COPY_ID",
                        text="Copy Parent Material Entry ID",
                    ).text = str(Entry.LoadedData.ParentMaterialID)
        # Draw copy ID buttons
        if SingleEntry:
            row.separator()
            row.operator(
                "helldiver2.copytest", icon="COPY_ID", text="Copy Entry ID"
            ).text = str(Entry.FileID)
            row.operator(
                "helldiver2.copytest", icon="COPY_ID", text="Copy Entry Hex ID"
            ).text = str(hex(Entry.FileID))
            row.operator(
                "helldiver2.copytest", icon="COPY_ID", text="Copy Type ID"
            ).text = str(Entry.TypeID)
            row.operator(
                "helldiver2.copytest", icon="COPY_ID", text="Copy Friendly Name"
            ).text = GetFriendlyNameFromID(Entry.FileID)
            if Global_TocManager.IsInPatch(Entry):
                props = row.operator(
                    "helldiver2.archive_entryrename", icon="TEXT", text="Rename"
                )
                props.object_id = str(Entry.FileID)
                props.object_typeid = str(Entry.TypeID)
        if Entry.IsModified:
            row.separator()
            props = row.operator(
                "helldiver2.archive_undo_mod", icon="TRASH", text=UndoName
            )
            props.object_id = FileIDStr
            props.object_typeid = TypeIDStr
        ...
        if SingleEntry:
            row.operator(
                "helldiver2.archive_setfriendlyname",
                icon="WORDWRAP_ON",
                text="Set Friendly Name",
            ).object_id = str(Entry.FileID)
    ...
    def draw(self, context):
        ...
        if menuName == "HELLDIVER2_OT_archive_entry":
            ...
            self.draw_entry_buttons(
                layout, Global_TocManager.GetEntry(int(FileID), int(TypeID))
            )
